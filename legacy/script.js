...
const DashboardAuth = {
  ...
  async authenticate(username, password) {
    try {
      // Retrieve CSRF token from meta/localStorage; fetch if missing
      ...
      if (!csrfToken) {
        try {
          ...
          if (tData && tData.csrfToken) {
            ...
            localStorage.setItem('justiceCsrfToken', csrfToken);
          }
        } catch {}
      }
  ...
  // Show login form (alias to single renderer)
  showLoginForm() {
    ...
    if (loginFormEl) {
      loginFormEl.addEventListener("submit", async (e) => {
        ...
        if (result && result.success) {
          ...
          if (err) {
            err.textContent = (result && result.error) || "Invalid credentials. Please try again.";
  ...
  if (errorEl && errorEl.classList) errorEl.classList.add("hidden");
    ...
    try {
      ...
      if (result.success) {
        // Success - render dynamic dashboard UI
        this.loadDashboard();
      } else {
        this.showLoginError(
