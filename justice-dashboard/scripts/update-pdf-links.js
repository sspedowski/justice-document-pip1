...
function displayPDFFiles(pdfs) {
    if (pdfs.length === 0) {
        ...
        log('\nTIP: You\'ll need to provide the full absolute path to your PDF file.', 'yellow');
        return;
    }
    ...
    log('\nTIP: Copy and paste one of the paths above, or provide a custom path.', 'yellow');
}
...
function runPythonScript(inputPath, outputPath) {
    return new Promise((resolve, reject) => {
        ...
        if (!fs.existsSync(pythonScript)) {
            ...
            log('Please ensure update_pdf_links.py exists in the project root.', 'gray');
...
async function main() {
    ...
    if (!inputPath) {
        ...
        log('\nExample:', 'yellow');
        log('  node scripts/update-pdf-links.js "server/uploads/document.pdf"', 'gray');
        log('  npm run update-pdf-links "C:/Documents/legal.pdf" "Updated_Legal.pdf"', 'gray');
