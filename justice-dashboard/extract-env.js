...
targetFiles.forEach(file => {
    ...
    patterns.forEach(pattern => {
        ...
        while ((match = regex.exec(content)) !== null) {
            ...
            const envKey = file.includes('frontend') || file.includes('client') ? 
                (key.startsWith('VITE_') ? key : `VITE_${key}`) : key;
...
if (envVars.has('DASH_USER')) envContent += `DASH_USER=${envVars.get('DASH_USER')}\n`;
if (envVars.has('DASH_PASS')) envContent += `DASH_PASS=${envVars.get('DASH_PASS')}\n`;

envContent += '\n# OpenAI API Configuration\n';
if (envVars.has('OPENAI_API_KEY')) envContent += `OPENAI_API_KEY=${envVars.get('OPENAI_API_KEY')}\n`;

envContent += '\n# Firebase Configuration (VITE_ prefix for client-side access)\n';
if (envVars.has('VITE_FIREBASE_API_KEY')) envContent += `VITE_FIREBASE_API_KEY=${envVars.get('VITE_FIREBASE_API_KEY')}\n`;
if (envVars.has('VITE_AUTH_DOMAIN')) envContent += `VITE_FIREBASE_AUTH_DOMAIN=${envVars.get('VITE_AUTH_DOMAIN')}\n`;
if (envVars.has('VITE_PROJECT_ID')) envContent += `VITE_FIREBASE_PROJECT_ID=${envVars.get('VITE_PROJECT_ID')}\n`;
if (envVars.has('VITE_STORAGE_BUCKET')) envContent += `VITE_FIREBASE_STORAGE_BUCKET=${envVars.get('VITE_STORAGE_BUCKET')}\n`;
if (envVars.has('VITE_MESSAGING_SENDER_ID')) envContent += `VITE_FIREBASE_MESSAGING_SENDER_ID=${envVars.get('VITE_MESSAGING_SENDER_ID')}\n`;
if (envVars.has('VITE_APP_ID')) envContent += `VITE_FIREBASE_APP_ID=${envVars.get('VITE_APP_ID')}\n`;
