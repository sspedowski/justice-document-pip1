 ...
 */
exports.onDocumentUpload = onDocumentCreated(
  ...
  async event => {
    try {
      ...
      if (docData.documentUrl && !docData.processed) {
        logger.info('Auto-processing uploaded document', {
        ...
        logger.info('Document auto-processing completed', {
          docId: event.params.docId,
        });
      }
    } catch (error) {
...
async function extractLegalStatutes(text) {
  ...
  if (
    ...
    lowerText.includes('14th amendment')
  ) {
    statutes.push('14th Amendment – Due Process');
  }
  if (
    lowerText.includes('1st amendment') ||
    lowerText.includes('free speech')
  ) {
    statutes.push('1st Amendment – Free Speech');
  }
  if (
    lowerText.includes('search warrant') ||
    lowerText.includes('4th amendment')
  ) {
  ...
  // Federal Statutes
  if (
    lowerText.includes('brady v. maryland') ||
    lowerText.includes('exculpatory') ||
    lowerText.includes('suppressed evidence')
  ) {
    statutes.push('Brady v. Maryland – Suppression of Evidence');
  }
  if (
    lowerText.includes('42 u.s.c. § 1983') ||
    lowerText.includes('civil rights') ||
    lowerText.includes('color of law')
  ) {
  ...
  if (
    lowerText.includes('722.628') ||
    lowerText.includes('cps duty') ||
    lowerText.includes('failure to investigate')
  ) {
    statutes.push('MCL 722.628 – CPS Duty to Investigate');
  }
  if (
    lowerText.includes('722.623') ||
    lowerText.includes('mandatory report') ||
    lowerText.includes('reporting requirements')
  ) {
    statutes.push('MCL 722.623 – Mandatory Reporting');
  }
  if (
    lowerText.includes('764.15c') ||
    lowerText.includes('retaliation') ||
    lowerText.includes('illegal retaliation')
  ) {
    statutes.push('MCL 764.15c – Illegal Retaliation');
  }
  if (
    lowerText.includes('600.1701') ||
    lowerText.includes('contempt') ||
    lowerText.includes('court misconduct')
  ) {
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are a legal expert analyzing documents for potential misconduct, constitutional violations, and due process issues. Focus on Michigan law (MCL) and federal constitutional rights.',
        },
        {
          role: 'user',
          content: prompt,
...
async function classifyDocument(text) {
  ...
  if (
    ...
    lowerText.includes('brady') ||
    lowerText.includes('evidence suppression')
  ) {
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are a legal research assistant. Extract all legal references from documents and return them as a clean JSON array of strings.',
        },
        {
          role: 'user',
          content: prompt,
