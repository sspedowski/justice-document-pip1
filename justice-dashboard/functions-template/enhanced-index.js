...
exports.analyzeDocument = onCall(async request => {
  ...
  try {
    ...
    const legalStatutes =
      await DocumentProcessor.identifyLegalStatutes(extractedText);
    await analysisRef.update({
      legalStatutes,
      progress: 75,
    });
    ...
    // Generate summary
    const summary = await DocumentProcessor.generateSummary(
      extractedText,
      options
    );
...
exports.getDocumentAnalysis = onCall(async request => {
  try {
    ...
    const documentData = await SecurityManager.checkDocumentAccess(
      auth.uid,
      analysisId
    );
...
exports.onDocumentUpload = onDocumentCreated(
  ...
  async event => {
    try {
      ...
      logger.info('Document upload detected', {
        analysisId,
        userId: data.userId,
      });
 ...
 */
exports.onAnalysisComplete = onDocumentUpdated(
  'document_analysis/{analysisId}',
  async event => {
    try {
      const analysisId = event.params.analysisId;
      ...
      if (
        ...
        await userStatsRef.set(
          {
            documentsProcessed:
              (await userStatsRef.get()).data()?.documentsProcessed + 1 || 1,
...
exports.getSystemStats = onCall(async request => {
  try {
    ...
    const stats = {
      ...
      totalDocuments: (
        await db.collection('document_analysis').count().get()
      ).data().count,
      documentsToday: (
        await db
          .collection('document_analysis')
