  ...
  async function checkServerStatus() {
    try {
      ...
      if (data.status === 'online') {
        if (serverIndicator) serverIndicator.className = 'w-2 h-2 rounded-full bg-green-400';
        if (serverStatus) serverStatus.textContent = 'Server Online';
        if (connectionStatus) connectionStatus.textContent = '';
      } else {
        if (serverIndicator) serverIndicator.className = 'w-2 h-2 rounded-full bg-yellow-400';
        if (serverStatus) serverStatus.textContent = 'Server Issue';
        if (connectionStatus) connectionStatus.textContent = 'API error';
      }
    } catch (err) {
      if (serverIndicator) serverIndicator.className = 'w-2 h-2 rounded-full bg-red-400';
